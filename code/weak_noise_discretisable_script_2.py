...
def run_script():
    ...
    pdf_path = os.path.join(output_dir, 'weak_noise_discretisable_models_analysis_2.pdf')

    seed = 42
    mean_simplicity_range = np.array([0.3, 0.4, 0.5, 0.6, 0.7])
    mean_weakness_range = np.array([0.3, 0.4, 0.5, 0.6, 0.7])
            ...
            for mean_weakness in mean_weakness_range:
                for num_endogenous_variables in num_endogenous_variables_range:
                    ...
                    for _ in range(num_samples_per_combination):
                        causal_model = create_causal_bayesian_network(num_endogenous_variables, mean_simplicity, mean_weakness)
